...
class Evaler(object):
    def __init__(self, eval_ids, gv_feat, att_feats, eval_annfile, max_samples=None, enable_eval_loss=False, corrupt_level="none"):
        super(Evaler, self).__init__()
        self.vocab = utils.load_vocab(cfg.INFERENCE.VOCAB)
        self.max_samples = max_samples
        self.enable_eval_loss = enable_eval_loss

        # 存储 corrupt_level，以便在构建 byteformer loader 时注入到其 opts
