...
def parse_args():
    ...
    parser.add_argument("--corrupt_level", type=str, default="none",
                        choices=["none", "light", "medium", "heavy"],
                        help="Apply corruption to image bytes for evaluation/training")
...
if __name__ == '__main__':
    ...
    # 将 corrupt_level 注入到 byteformer data_loader 使用的 opts（使 pil_save 读取到该值）
    try:
        import PureT.datasets_.data_loader_byteformer_coco as dlc_mod
        # 确保层次结构存在
        if not hasattr(dlc_mod.opts, "image_augmentation"):
            dlc_mod.opts.image_augmentation = argparse.Namespace()
        if not hasattr(dlc_mod.opts.image_augmentation, "pil_save"):
            dlc_mod.opts.image_augmentation.pil_save = argparse.Namespace()
        dlc_mod.opts.image_augmentation.pil_save.corrupt_level = getattr(args, "corrupt_level", "none")
        print(f"[INFO] Injected corrupt_level={dlc_mod.opts.image_augmentation.pil_save.corrupt_level} into data loader opts")
    except Exception:
        print("[WARNING] Failed to inject corrupt_level into data loader opts; pil_save will use default")
