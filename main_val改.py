...
class Tester(object):
    def __init__(self, args):
        ...
        # corrupt_level 从命令行透传（最小修复：确保已定义）
        ...
        self._print_config_summary(args)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        if self.dataset_type == "coco":
            ...
            # 传入 corrupt_level 到 Evaler / 内部 loader，使 apply_pil_save 能读取并调用
...
def parse_args():
    ...
    parser.add_argument("--corrupt_level", type=str, default="none", choices=["none","light","medium","heavy"],
